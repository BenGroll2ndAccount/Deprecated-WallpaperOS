...
class uConstrain():
    def __init__(self, pointA : uPoint, pointB : uPoint, polypoints = None):
    ...
    @property
    def copy(self):
        return uConstrain(pointA=uPoint(x = self.pointA.x, y = self.pointA.y), pointB=uPoint(x = self.pointB.x, y = self.pointB.y), polypoints=self.polypoints.copy)
    ...
    def isSafe(self, other_const, name):
        if self.polypoints == None and other_const.polypoints == None:
